...
namespace FastFood.DataProcessor
{
	public static class Deserializer
	{
		...
		public static string ImportEmployees(FastFoodDbContext context, string jsonString)
		{
		    ...
		    foreach (var deserializedEmployee in deserializedEmployees)
		    {
		        if (!IsValid(deserializedEmployee))
		        {
		            sb.AppendLine(FailureMessage);
                    continue;
		        }

		        Position position = GetPosition(context, deserializedEmployee.Position);
                var employee = new Employee
                {
                    Name = deserializedEmployee.Name,
                    Age = deserializedEmployee.Age,
                    Position = position
                };
	    ...
	    private static Position GetPosition(FastFoodDbContext context, string deserializedEmployeePosition)
	    {
	        var position = context.Positions.FirstOrDefault(x => x.Name == deserializedEmployeePosition);
	        if (position == null)
	        {
	            position = new Position
	            {
	                Name = deserializedEmployeePosition
	            };
	    ...
	    private static bool AreValidItems(FastFoodDbContext context, OrderItemDto[] orderDtoOrderItems)
	    {
	        foreach (var item in orderDtoOrderItems)
	        {
